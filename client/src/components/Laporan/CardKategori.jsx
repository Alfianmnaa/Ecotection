...
const CardKategori = ({ selectedCategory }) => {
  ...
  useEffect(() => {
    fetchLaporan();
  }, [selectedCategory]); // Menambahkan kategori sebagai dependency

  const fetchLaporan = async () => {
    try {
      if (selectedCategory) {
        setLoading(true);
        const res = await axiosInstance.get(`/aksi/laporan/kategori?kategori=${selectedCategory}`);
        setLoading(false);
        setSemuaData(res.data);
        semuaData.forEach((laporan) => {
  ...
  return (
    <section>
      {loading ? (
        <div className="skeleton-wrapper cardContainer flex flex-wrap justify-center gap-8">
          {[1, 2, 3].map((index) => (
